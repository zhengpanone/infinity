...
//! 复用 `crate::http::shutdown_signal` 做优雅关闭，错误统一收敛为
//! [`InfinityError`](infinity_error::InfinityError)。

use std::net::SocketAddr;

use infinity_config::AppConfig;
...
pub async fn serve(config: &AppConfig) -> Result<()> {
    ...
    Server::builder()
        ...
        .serve_with_shutdown(addr, crate::http::shutdown_signal())
        .await
        .with_context(ErrorKind::Web, || format!("admin gRPC server error on {addr}"))?;
